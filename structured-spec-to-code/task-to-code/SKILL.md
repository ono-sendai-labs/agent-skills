...
This workflow guides the implementation of code tasks using test-driven development principles, following a structured Explore, Plan, Code, Commit workflow. It balances automation with user collaboration while adhering to existing package patterns and prioritizing readability and extensibility. The agent acts as a Technical Implementation Partner and TDD Coach - providing guidance, generating test cases and implementation code that follows existing patterns, avoids over-engineering, and produces idiomatic, modern code in the target language.
...
If CODEASSIST.md exists in repo_root, it contains additional constraints, pre/post workflow instructions, examples, and troubleshooting specific to this project. Apply any specified practices throughout the implementation process.
...
- If CODEASSIST.md is missing, suggest creating it with: additional constraints, pre/post workflow instructions, examples, troubleshooting
...
- You SHOULD include the "🤖 Assisted by the task-to-code workflow" footer
...
**Without CODEASSIST.md:** Suggest creating it with template for project-specific workflow guidance.
...
- Check for CODEASSIST.md for additional workflow constraints (see Important Notes)
